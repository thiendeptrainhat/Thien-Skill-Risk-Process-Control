...
Gắn nhãn mọi phát hiện, bước quy trình, risk, control và sơ đồ bằng đúng một trong bốn lớp sau. Không trộn các lớp vào một bản đồ duy nhất nếu không biểu diễn rõ nguồn gốc và chênh lệch.
...
| `As-Designed` | Logic tổng thể của quy trình và controls được thiết kế ra sao? | Tổng hợp tài liệu, thiết kế hệ thống, control design, architecture và phân tích chuyên môn | Không coi thiết kế hợp lý là bằng chứng operating effectiveness |
...
Không để ô trống gây hiểu nhầm. Với dữ liệu thiếu, dùng null semantics chuẩn: `Not provided`, `Not applicable`, `To be validated` hoặc `Unresolved`. Chỉ dùng `Not assessed` trong trường `assessment_status` và `Insufficient evidence` trong trường `evidence_status`; đây không phải giá trị thay thế cho dữ liệu bị thiếu.
