...
description: Phân tích và thiết kế quy trình, policy/SOP, risk, controls, RCM, SoD, SPOF và target state. Use for process discovery, mapping, control gaps, workflow, or audit handoff.
...
Route phần việc thuần legal opinion, full audit engagement, fraud investigation, BIA/BCP, data engineering, ERP configuration, production workflow programming, penetration testing hoặc UI design sang năng lực phù hợp nếu có. Nếu không có năng lực đó, giới hạn output ở phần process–risk–control và nêu handoff cần thiết.

## Bốn lớp phân tích

Gắn một trong bốn nhãn sau cho từng process step, finding, diagram và recommendation:

- `As-Documented`: điều tài liệu hiện hành quy định.
- `As-Designed`: logic thiết kế tổng thể, kể cả thiết kế chưa được tài liệu hóa đầy đủ.
...
- Xác minh nguồn hiện hành từ nguồn chính thức khi người dùng yêu cầu standard, best practice hoặc compliance mapping. Nếu không thể xác minh, đánh dấu `To be validated`; không trình bày recommendation như nghĩa vụ.
...
Với engagement đầy đủ, đi theo đúng chuỗi 14 bước dưới đây. Các mục chi tiết kế tiếp nhóm kỹ thuật để tra cứu, không thay đổi thứ tự này. Entry/success/stop gate và output của từng bước nằm tại [data-model-qa-execution.md](references/data-model-qa-execution.md#9-workflow-chuẩn-14-bước).
...
3. Lập source register, version/effective status, owner, conflict và missing source.
4. Dựng process architecture L0–L5 mà nguồn cho phép; không tự gán level.
...
7. Phân tích control objective, current/key/supporting control, design, evidence và testability.
8. Map external requirement hoặc best practice sau khi xác minh applicability và mandatory/advisory status.
9. Phân tích gap, SoD, SPOF/dependency, over-control, duplicate và bottleneck.
10. Thiết kế target-state options, governance, metrics và implementation dependencies.
...
Phân loại missing/weak/unclear/untestable/duplicated/redundant/over-control và operating deviation. Tách observation, design opportunity, control gap, compliance gap, potential finding và confirmed finding.
