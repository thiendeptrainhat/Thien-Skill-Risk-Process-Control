...
| Trạng thái | Phase 3 — đang kiểm thử/chuẩn bị phát hành; chưa kết luận nghiệm thu |
| Repository | Private |
| Ngôn ngữ trả lời | Theo ngôn ngữ người dùng |
| License | Tran Ngoc Thien's Skills Commercial Source-Available License 2.0 |

Tên ZIP bên dưới là mục tiêu của bản `1.1.0`. Chỉ dùng gói khi file, version và checksum tương ứng đã có trong [release manifest](RELEASE-MANIFEST.yaml). Không dùng kết quả kiểm thử `1.0.0` để suy ra `1.1.0` đã đạt.

## Cài đặt

### Chọn đúng ZIP

| Gói mục tiêu | Bên trong ZIP | Dùng như thế nào |
|---|---|---|
...
Trạng thái `1.1.0` đang được hoàn thiện trong Phase 3. Không công bố số case pass hoặc platform accepted trước khi có run evidence tương ứng.
