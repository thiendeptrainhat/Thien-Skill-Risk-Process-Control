...
Ba ZIP `1.1.0` bên dưới đã được tạo và đối chiếu với cùng canonical snapshot. Version và checksum nằm trong [release manifest](RELEASE-MANIFEST.yaml); kết quả và giới hạn tại [báo cáo Phase 3](docs/phase-3/REPORT.md). Đây là artifact local, chưa được commit/push hoặc công bố lên GitHub trong Phase 3. Không dùng kết quả kiểm thử `1.0.0` để suy ra `1.1.0` đã đạt.
